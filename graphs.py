...
trimmed_mean_female_el = [main.trimmed_mean_bachelor_female, main.trimmed_mean_master_female, main.trimmed_mean_phd_female]
...
ax4.bar(education_level, trimmed_mean_female_el, bottom=trimmed_mean_male_el, color=colors_adjusted[1], label="Master's")
ax4.bar(education_level, trimmed_mean_total_el, bottom=[sum(x) for x in zip(trimmed_mean_male_el, trimmed_mean_female_el)],
        color=colors_adjusted[2], label="PhD")
...
fig2a, (ax1, ax2) = plt. subplots(1, 2, figsize=(10, 6))
...
ax1.set_xticklabels(['Male', 'Female', 'Total'])
...
#ax1.text(0.0, -0.15, f'Standard Error male: {main.standard_error_male}\nStandard Error female: {main.standard_error_female}\nStandard Error total: {main.standard_error_total}', transform=ax1.transAxes, fontsize=8)
ax1.text(0.25, -0.10, f"Share of male: {main.share_male}%  -  Share of female: {main.share_female}%", transform=ax1.transAxes, fontsize=8)
ax2.text(0.08, -0.10, f"Share of Bachelor's: {main.share_bachelor}%  -  Share of Master's: {main.share_master}%  -  Share of PhD: {main.share_phd}%", transform=ax2.transAxes, fontsize=8)
...
fig2b, (ax3, ax4) = plt. subplots(1, 2, figsize=(10, 6))

sns.kdeplot(data=main.salary_data, x='Salary', hue='Gender', fill=True, ax=ax3, palette=palette_g)
sns.kdeplot(data=main.salary_data, x='Salary', hue='Education Level', fill=True, ax=ax4, palette=degree_colors)

plt.suptitle('Overlapping density plot of salary', fontsize=14, fontweight='bold')
ax3.set_title('Salary for men and women: Mostly in agreement', fontsize=12)
ax4.set_title('Income largely depends on the degree', fontsize=12)

ax3.set_xlabel('Salary ($)', fontsize=9, fontweight='bold')
ax3.set_ylabel('Density', fontsize=9, fontweight='bold')
ax4.set_xlabel('Salary ($)', fontsize=9, fontweight='bold')
...
ax3.legend(labels=labels_legend, loc='upper right', fontsize='small', title=None)
ax4.legend(labels=education_level_s, loc='upper right', fontsize='small', title=None)
...
fig3, (ax1, ax2) = plt. subplots(1, 2, figsize=(10, 6))
...
# sns.boxplot(data=main.salary_data, y='Salary', ax=ax2)
